...
class PayerConfigService {
  ...
  async getProcedureHandling(
    ...
    try {
      ...
      // Get procedure overrides for this payer
      const overrides = await this.getProcedureOverrides(config.id);
      
      // Check if there's an override for this procedure code
      ...
      // Use the default values from the payer config
      return {
